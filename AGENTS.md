...
3. Если код невозможно покрыть напрямую из-за VS Code API, вынести чистую логику в `domain/` или `infra/` и покрыть её unit-тестом; тонкий UI-адаптер проверить интеграционным тестом через `@vscode/test-electron`.
4. Перед завершением задачи обязательно выполнить `npm test` и `npm run coverage`. Если команда не может быть выполнена локально, зафиксировать конкретную причину и не считать задачу завершённой.
